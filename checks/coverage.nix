...
let
  ...
  partial = {
    "RADIO-2" = "the RANKING is fully exercised on a booted machine (checks/vm/wireless-outranks-modem.nix: two real default routes at the derived metrics, the kernel picking the lower one, and a control node proving the numbers are not NetworkManager's own). What is stood in for is the HARDWARE: both radios are ordinary NetworkManager-managed interfaces, one declared `wifi` and one declared `wwan`. That is sound for the mechanism -- a per-device route-metric default is device-type agnostic, and it has to be, since a modem's own connection is ModemManager's to create -- but no test in this repo has ever ranked a REAL modem against a REAL Wi-Fi association. The Wi-Fi half of that is covered separately by RADIO-1, which associates over mac80211_hwsim; the modem half is untested for want of a modem";
    "RADIO-3" = "all three policy outcomes are exercised on a booted machine (checks/vm/wireless-power-policy.nix), and the switch under test -- NetworkManager's per-KIND radio state -- is a real property change, rfkill-backed, read back from NetworkManager. Two gaps, both stated rather than implied: the radios are stood in by ordinary interfaces (same fixture note as RADIO-2), and the POWER SOURCE is passed to the script as an argument rather than detected, because a VM has no battery to unplug. So the policy is proven and the sysfs reading that chooses between its two halves is not, nor is the udev rule that re-runs it on a real cable change";
